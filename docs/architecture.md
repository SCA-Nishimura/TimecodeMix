...
| 配布 | GitHub Pages / Vercel | USBメモリ(ポータブル形式) |
| インストール | 不要 | 不要(フォルダをコピーするだけ) |
...
| コマンド | 引数 | 返り値 | 用途 |
|---|---|---|---|
| `check_ffmpeg` | なし | バージョンと参照元 | 起動時の環境確認 |
...
どちらを使っているかは起動時に画面上部へ表示される(`(同梱)` / `(システム)`)。
配布後に「動かない」と言われたとき、同梱バイナリが欠けているのかを切り分けられるようにしている。

### ffmpegの同梱方法

バイナリは ffmpeg / ffprobe あわせて約460MBあるため、**Gitには入れていない**。
ビルド前にセットアップスクリプトで配置する。

```bash
node scripts/setup-ffmpeg.mjs
```

システムにインストール済みの ffmpeg / ffprobe を、Tauriのサイドカーが要求する
`<名前>-<ターゲットトリプル>` という命名で `desktop/src-tauri/binaries/` へ複製する。
バンドル時にトリプル部分は取り除かれ、実行ファイルの隣に置かれる。

USB配布を前提にしているため、**アプリ単体で完結すること**が要件になる。
利用者の環境に ffmpeg をインストールさせる前提にはしない。
...
開発中は PATH 上の ffmpeg を使うため、同梱の準備は不要。
...
`dist-portable/TimecodeMix/` に以下が出力される。このフォルダをUSBにコピーして配布する。
...
**インストーラは作らない。** USB配布を前提にしているため、フォルダをコピーして
実行ファイルを起動するだけで済むポータブル形式にしている。
インストール不要・管理者権限不要で、USB経由なら署名が無くてもSmartScreenの警告が出ない
(警告はブラウザでのダウンロード時に付くMOTWが原因のため)。
